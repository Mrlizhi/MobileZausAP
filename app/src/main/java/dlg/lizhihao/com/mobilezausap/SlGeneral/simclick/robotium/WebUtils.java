...
class WebUtils {
	...
	RobotiumWebViewClient robotiumWebViewClient;
	...
	public WebUtils(Solo.Config config, Instrumentation instrumentation, ViewFetcher viewFetcher, Sleeper sleeper){
		...
		robotiumWebViewClient = new RobotiumWebViewClient(instrumentation);
	}
	...
	private String prepareForStartOfJavascriptExecution(List<WebView> webViews) {
		...
		if (currentWebViewClient != null && !currentWebViewClient.getClass().isAssignableFrom(RobotiumWebViewClient.class)){
...
//		robotiumWebViewClient.setRobotiumWebviewClient(webViews, originalWebViewClient);
	...
	private String getJavaScriptAsString() {
		//update by sogood
		return "function allWebElements() {\n" +
